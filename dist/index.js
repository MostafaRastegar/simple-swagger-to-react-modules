...
program
    ...
    try {
        ...
        for (const [definitionName] of Object.entries(swaggerData.definitions || {})) {
        ...
        const modelFileNames = Object.keys(interfaceFiles).map((f) => path.basename(f));
        // 5. Generate Service Classes
        console.log("Generating service classes...");
        const serviceFiles = {};
        const serviceNames = []; // Explicitly type as string array
        for (const [pathVal, pathItem] of Object.entries(swaggerData.paths || {})) {
            ...
            for (const [method, operation] of Object.entries(pathItemOperations)) {
                if ([
                    "get",
                    "post",
                    "put",
                    ...
                    const serviceName = llmGenerator.generateMethodName(pathVal, method, operation);
                    serviceNames.push(serviceName);
                    console.log(`  - Generating service method for: ${serviceName} (${method.toUpperCase()} ${pathVal})`);
                    const prompt = llmGenerator.generateServiceMethodPrompt(swaggerData, pathVal, method, operation);
                    ...
                    if (!existingServiceCode.includes(`export class ${serviceName}Service`)) {
                        serviceFiles[serviceFileName] = classDeclaration;
                    }
                    else {
                        console.warn(`Warning: Service file ${serviceFileName} already exists. Overwriting with new class for ${serviceName}. Consider grouping by tag.`);
        ...
        const serviceFileNames = Object.keys(serviceFiles).map((f) => path.basename(f));
        ...
        console.log('  3. Configure axios base URL in your application if needed (e.g., axios.defaults.baseURL = "YOUR_API_URL");');
