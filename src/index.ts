...
program
  ...
  .option(
    "-t, --tags <tags>",
    "Comma-separated list of tags to generate (e.g., 'pet,store')"
  )
  .action(
    async (
      swaggerFile: string,
      ...
      try {
        ...
        if (options.tags) {
          ...
          const selectedTagsInput = await new Promise<string>((resolve) => {
            rl.question(
              "Enter the numbers of the tags you want to generate (comma-separated, e.g., 1,3,4): ",
              (answer: string) => {
                resolve(answer);
              }
            );
          ...
          console.log(
            `Selected tags for generation: ${selectedTagNames.join(", ")}`
          );
        }

        // 6. Group Swagger data by tags
        console.log("\nGrouping Swagger data by tags...");
        const allTagGroups: TagGroupData[] =
          groupSwaggerDataByTags(swaggerData);
        console.log(
          `Found ${allTagGroups.length} tag groups in Swagger: ${allTagGroups
            .map((tg) => tg.tagName)
            .join(", ")}`
        );

        const selectedTagGroups: TagGroupData[] = allTagGroups.filter(
          (tagGroup) => selectedTagNames.includes(tagGroup.tagName)
        );
        ...
        console.log(
          ...
          } tag groups for generation: ${selectedTagGroups
            .map((tg) => tg.tagName)
            .join(", ")}`
        );
